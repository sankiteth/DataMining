...
def transform_Gaussian(X):
    ...
    gamma = 70
    mu = np.zeros(x_cols)
    sigma = np.zeros( (x_cols,x_cols) ) + ( 0.0 * np.ones( (x_cols,x_cols) ) )
...
def transform_Laplace(X):
    ...
    loc = 0.0
    scale = 1.0
    w = np.random.laplace(loc, scale, (m, 400)) * np.sqrt(2 * gamma)
    b = np.random.uniform(0, 2 * np.pi, size=m)
    b = np.tile(b.T, (x_lines,1))
    result = np.cos(np.dot(X, w.T) + b) * np.sqrt(2.0/m)
